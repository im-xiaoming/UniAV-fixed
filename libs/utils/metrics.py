...
class ANETdetection(object):
    ...
    def evaluate(self, preds, default_gpu, verbose=True):
        ...
        if verbose:
            # print the results
            if default_gpu:
                logger.info('[RESULTS] Action detection results on {:s}.'.format(
                    self.dataset_name)
                )
                block = ''
                ...
                logger.info('Avearge mAP: {:.2f} (%)'.format(average_mAP*100))
