...
from .task_utils import ForwardModelsTrain
...
def save_checkpoint(state, task_name, is_best, file_folder,
    ...
    if is_best:
        ...
        file_name = "%s_model_best.pth.tar" % (task_name)
...
def make_optimizer(model, optimizer_config, base_lr):
    """create optimizer
    ...
    inter_params = decay & no_decay & clip_params
    union_params = decay | no_decay | clip_params
    ...
    optim_groups = [
        {"params": [param_dict[pn] for pn in sorted(list(decay))], 
         "weight_decay": optimizer_config['weight_decay'], "lr": base_lr},
        {"params": [param_dict[pn] for pn in sorted(list(no_decay))], 
         "weight_decay": 0.0, "lr": base_lr},
        {"params": [param_dict[sorted(list(clip_params))[0]]], 
         "weight_decay": 0.0, "lr": optimizer_config["clip_proj_lr"]},
        {"params": [param_dict[sorted(list(clip_params))[1]]], 
         "weight_decay": optimizer_config['weight_decay'], "lr": optimizer_config["clip_proj_lr"]},
        ]
...
def make_scheduler(
    ...
    if optimizer_config["warmup"]:
        max_epochs = num_epochs + optimizer_config["warmup_epochs"]
...
def train_one_epoch_multitask(
    ...
    model.train()
    if default_gpu:
        logger.info("\n[Train]: Epoch {:d} started".format(epochId))
    start = time.time()
    for step in range(median_num_iter):
            iterId = step + (epochId * median_num_iter)
            first_task = True
            for task_id in task_ids:
                is_forward = False
                if (not task_stop_controller[task_id].in_stop) or (
                    iterId % cfg['train_iter_gap'] == 0):
                    is_forward = True

                if is_forward:
                    # zero out optim 
                    optimizer.zero_grad(set_to_none=True)
                    losses = ForwardModelsTrain(
                        cfg,
                        task_cfg,
                        device,
                        task_id,
                    ...
                    losses['final_loss'] = losses['final_loss'] * loss_scale[task_id]  
                    losses['cls_loss'] = losses['cls_loss'] * loss_scale[task_id]
                    losses['reg_loss'] = losses['reg_loss'] * loss_scale[task_id]
                    losses['final_loss'].backward()
                    # gradient cliping (to stabilize training if necessary)
                    if clip_grad_l2norm > 0.0:
                        torch.nn.utils.clip_grad_norm_(
                    ...
                    if default_gpu:
                        tb_logger.step_train(
                            ...
                            'train')
            scheduler.step() 
            if (step % print_freq) == 0 and (step != 0) and default_gpu:
    ...
    if default_gpu:
        logger.info("[Train]: Epoch {:d} finished with lr={:.8f}, Total time: {:0.2f} sec\n"
                    .format(epochId, lr, end - start))
    return 

def valid_one_epoch_multitask(
    val_loader,
    task_cfg,
    task_ids,
    model,
    ...
    # set up meters
    batch_time = AverageMeter()
    task_mAPs = {task_id: 0.0 for task_id in task_ids}
    # loop over validation set
    start = time.time()
    for task_id in task_ids:
        if default_gpu:
            print("\nStart testing on {:s} for {:s} task..."
                  .format(task_cfg[task_id]['dataset_name'], task_cfg[task_id]['task_type']))
        ...
        for iter_idx, video_list in enumerate(val_loader[task_id], 0):
            # forward the model (wo. grad)
            with torch.no_grad():
                output = model(video_list, task_id, task_cfg[task_id]['task_type'])
                if tb_writer is not None:
                    tb_writer.step_val(curr_epoch, task_id, len(video_list))
            ...
            if (iter_idx != 0) and iter_idx % (print_freq) == 0:
                ...
                if default_gpu:
                    ...
                    'Time {batch_time.val:.2f} ({batch_time.avg:.2f})'.format(
                        iter_idx, len(val_loader[task_id]), batch_time=batch_time))
        ...
        if evaluator is not None:
            ext_score_file = task_cfg[task_id]['test_cfg']['ext_score_file']
            if (ext_score_file is not None) and isinstance(ext_score_file, str):
                results = postprocess_results(results, ext_score_file)
                
            # call the evaluator
            _, task_mAPs[task_id] = evaluator[task_id].evaluate(results, default_gpu, verbose=True)
        else:
            # dump to a pickle file that can be directly used for evaluation
            with open(output_file, "wb") as f:
                pickle.dump(results, f)
        ...
        if tb_writer is not None:
            ...
            #mode=max,check mAP
            task_stop_controller[task_id].step(task_mAPs[task_id])
            tb_writer.showLossVal(task_id, curr_epoch, 
                                  task_mAPs[task_id], task_stop_controller)
